...
class ExperimentBuilder(nn.Module):
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        # Collect the absolute mean of gradients for each layer and store the layer name
        for name, param in named_parameters:
            if param.requires_grad and param.grad is not None:
                layers.append(name)  # Add layer name
                all_grads.append(param.grad.abs().mean().item())  # Add the mean of the absolute gradient
            ########################################
